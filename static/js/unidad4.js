// ===============================
// 🌙 RETOMATE - NÚMEROS FRACCIONARIOS
// Aventura Nocturna del Zorrito con 10 Cofres y Casita Final
// ===============================
...
// ESTRELLAS para el fondo nocturno
let estrellas = [];
for (let i = 0; i < 100; i++) {
  estrellas.push({
...
function drawBackground() {
  ...
  // Brillo de la luna
  const moonGlow = ctx.createRadialGradient(moonX, moonY, 10, moonX, moonY, moonRadius + 30);
...
function drawCofres() {
  cofres.forEach((c) => {
    ...
    ctx.fillStyle = c.abierto ? "#5a5a7a" : "#2a2a4a";
...
function drawCasita() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    ctx.fillStyle = `rgba(150, 150, 180, ${0.4 - i * 0.1})`;
    ctx.beginPath();
...
// ========== PREGUNTAS DE NÚMEROS FRACCIONARIOS ==========
const preguntas = [
  { pregunta: "¿Cuál es el resultado de 1/2 + 1/4?", opciones: ["2/6", "3/4", "2/4"], correcta: 1 },
  { pregunta: "¿Cuál fracción es equivalente a 2/4?", opciones: ["1/2", "3/6", "4/8"], correcta: 0 },
  { pregunta: "¿Cuál es el resultado de 3/5 - 1/5?", opciones: ["2/5", "4/10", "2/10"], correcta: 0 },
  { pregunta: "¿Cuál fracción es mayor: 3/4 o 2/3?", opciones: ["3/4", "2/3", "Son iguales"], correcta: 0 },
  { pregunta: "¿Cuál es el resultado de 2/3 × 3/4?", opciones: ["1/2", "5/7", "6/12"], correcta: 0 },
  { pregunta: "Simplifica la fracción 6/9", opciones: ["2/3", "3/4", "1/2"], correcta: 0 },
  { pregunta: "¿Cuál es el resultado de 5/6 - 1/3?", opciones: ["1/2", "4/3", "2/6"], correcta: 0 },
  { pregunta: "¿Cuánto es 1/4 de 20?", opciones: ["5", "10", "4"], correcta: 0 },
  { pregunta: "¿Cuál fracción es equivalente a 0.75?", opciones: ["3/4", "2/3", "4/5"], correcta: 0 },
  { pregunta: "¿Cuál es el resultado de 1/2 ÷ 1/4?", opciones: ["2", "1/8", "4"], correcta: 0 }
];
...
function mostrarPregunta(indiceCofre) {
  ...
  if (darSuperSalto) {
    ...
    if (siguienteCofreAlto !== undefined) {
      mensajePremio = `<p style="color:#64ffda; font-weight:bold; margin:10px 0;">¡Premio: SUPER SALTO! ⚡<br><small style="color:#a0a0d0;">Para alcanzar el cofre ${siguienteCofreAlto + 1}</small></p>`;
    } else {
      mensajePremio = '<p style="color:#64ffda; font-weight:bold; margin:10px 0;">¡Premio: SUPER SALTO! ⚡</p>';
    }
  } else {
    mensajePremio = '<p style="color:#64ffda; font-weight:bold; margin:10px 0;">¡Respuesta correcta! +10 puntos</p>';
  }
  
  contenedor.innerHTML = `
    <h3 style="margin:0 0 20px; color: #ffd700; font-size: 20px;">🌙 Cofre ${indiceCofre + 1}/10</h3>
    ${mensajePremio}
    <p style="margin: 0 0 20px; font-size: 18px; color: #e0e0ff;">${q.pregunta}</p>
    <div id="opcionesBox" style="display:flex;flex-direction:column;gap:12px;">
      ${q.opciones.map((op, idx) => `<button class='opcion' data-idx='${idx}' style="padding:12px 20px; font-size:16px; cursor:pointer; background:#5a7a9a; color:white; border:none; border-radius:5px; transition: all 0.2s;">${op}</button>`).join("")}
    </div>
  `;
  document.body.appendChild(contenedor);

  document.querySelectorAll(".opcion").forEach(btn => {
    btn.addEventListener("mouseover", (e) => e.target.style.background = "#4a6a8a");
    btn.addEventListener("mouseout", (e) => e.target.style.background = "#5a7a9a");
